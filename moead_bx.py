...
def geneticmoead(y, weight, zmin, popSon, sonSize, popSize, partern, distanceIndex, iteration, F, cr, trainDate, trainTag, testDate, testTag, feature, passageway, trainRow, testRow, feature_low, passageway_low,time_data):
    for i in range(iteration):
        ...
        index = [random.randint(0, popSize-1) for i in range(sonSize)]  # 必定变异位置
        ...
        for j in range(sonSize):
            if sum(t[j, 0:feature]) >= feature_low and sum(t[j, feature:popSize]) >= passageway_low:
                ynow = fitness(t[j, :], trainDate, trainTag, testDate,
                               testTag, feature, passageway, trainRow, testRow)
                if max(abs(weight[j, 0]*(ynow[0]-zmin[0]))/(feature-feature_low), abs(weight[j, 1]*(ynow[1]-zmin[1]))/(passageway-passageway_low), abs(weight[j, 2]*(ynow[2]-zmin[2])), abs(weight[j, 3]*(ynow[3]-zmin[3]))) < max(
                        abs(weight[j, 0]*(y[j, 0]-zmin[0]))/(feature-feature_low), abs(weight[j, 1]*(y[j, 1]-zmin[1]))/(passageway-passageway_low), abs(weight[j, 2]*(y[j, 2]-zmin[2])), abs(weight[j, 3]*(y[j, 3]-zmin[3]))):
                    popSon[j, :] = t[j, :]
                    y[j, :] = ynow
                    update_time += 1
    ...
    with open('result_moead'+str(feature_low)+'_'+str(passageway_low)+'_'+str(time_data)+'.txt', 'w') as f:
        for i in range(sonSize):
    ...
    ploturf(y, sonSize, feature_low, passageway_low,time_data)
    with open('result_moead_accuracy'+str(feature_low)+'_'+str(passageway_low)+'_'+str(time_data)+'.txt', 'w') as f:
        for i in range(sonSize):
            f.write(str(y[i, :]))
            f.write('\n')

def ploturf(y, sonSize, feature_low, passageway_low,time_data):
    ...
    plt.show()
    plt.savefig('moead'+str(feature_low)+'_'+str(passageway_low)+'_'+str(time_data)+'.png', dpi=500)
...
def main_deal(time_data):
    ...
    for i in range(5):  # 动作为5个
        for j in range(int(trainRow/5)):  # 标签为390个
            trainTag.append(i+1)
        for j in range(int(testRow/5)):
            testTag.append(i+1)
    ...
    print('begin train')
    print(time.ctime())
    geneticmoead(y, weight, zmin, popSon, sonSize, popSize, partern, distanceIndex, iteration,
                 F, cr, trainDate, trainTag, testDate, testTag, feature, passageway, trainRow, testRow, feature_low, passageway_low,time_data)
    print(time.ctime())

if __name__ == '__main__':
    pool = mp.Pool(processes=5)
    purchase_loss_value = pool.map(main_deal, range(5))
    pool.close()
    pool.join()