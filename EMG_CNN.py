...
# 读取肌电数据
def LoadData(p1,p2):
    X_train_orig = np.zeros((40950,300,6,1))   #44850
    X_test_orig = np.zeros((1,40950),dtype=int)
    Y_train_orig = np.zeros((5850,300,6,1))   #5850
    ...
    for files1 in os.listdir(p1):
        ...
        data = EMG['data']
        for i in range(195):
            X_train_orig[m * 195 + i, :, :, 0] = data[i*50:i*50+300,np.array([3,7,8,9,11,15])]
    ...
    for files2 in os.listdir(p2):
        ...
        data = EMG['data']
        for i in range(195):
            Y_train_orig[n * 195 + i, :, :, 0] = data[i*50:i*50+300,np.array([3,7,8,9,11,15])]
...
# reshape the output from the third convolution for the fully connected layer
    YY = tf.reshape(Y4, shape=[-1, 64*75*2])

#5:全连接层
    W5 = tf.Variable(tf.truncated_normal([64*75*2, 512], stddev=0.1))
