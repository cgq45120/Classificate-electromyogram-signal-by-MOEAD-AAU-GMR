...
def geneticmoead(y, weight, zmin, popSon, sonSize, popSize, partern, distanceIndex, iteration, F, cr, trainDate, trainTag, testDate, testTag, feature, passageway, trainRow, testRow, feature_low, passageway_low, time_data):
    weight = 1/weight
    Gmax = 5
    normailzation = numpy.array(
        [feature-feature_low, passageway-passageway_low, 1, 1])
    ...
    with open('result_moeadau'+str(feature_low)+'_'+str(passageway_low)+'_'+str(time_data)+'.txt', 'w') as f:
        for i in range(sonSize):
    ...
    with open('result_moeadau_accuracy'+str(feature_low)+'_'+str(passageway_low)+'_'+str(time_data)+'.txt', 'w') as f:
        for i in range(sonSize):
            f.write(str(y[i, :]))
            f.write('\n')
...
def ploturf(y, sonSize, feature_low, passageway_low, time_data):
    ...
    plt.savefig('moead_au'+str(feature_low)+'_' +
                str(passageway_low)+'_'+str(time_data)+'.png', dpi=500)
...
def main_deal(time_data):
    ...
    geneticmoead(y, weight, zmin, popSon, sonSize, popSize, partern, distanceIndex, iteration, F, cr, trainDate,
                 trainTag, testDate, testTag, feature, passageway, trainRow, testRow, feature_low, passageway_low, time_data)
